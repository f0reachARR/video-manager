...
import { useMarkers, useRobots, useRun, useScenarios } from "../../lib/queries";
...
function SharedRunPage() {
  ...
  if (run.error || !run.data) {
    return (
      <Alert color="red">
        {run.error instanceof ApiError ? run.error.body.message : String(run.error)}
      </Alert>
    );
  }
  const r = run.data;
  const robotName = (robots.data?.data ?? []).find((x) => x.id === r.robotId)?.name ?? r.robotId.slice(0, 8);
  const scenarioName = (scenarios.data?.data ?? []).find((x) => x.id === r.scenarioId)?.name ?? r.scenarioId.slice(0, 8);
  ...
  return (
    <Stack maw={960} mx="auto">
      <Group justify="space-between">
        ...
        <Badge color="grape" variant="light">read-only</Badge>
      </Group>
      ...
      <Card withBorder>
        <Stack>
          <Group justify="space-between">
            <Stack gap={2}>
              <Text size="xs" c="dimmed">Robot</Text>
              <Text fw={500}>{robotName}</Text>
            </Stack>
            <Stack gap={2}>
              <Text size="xs" c="dimmed">Scenario</Text>
